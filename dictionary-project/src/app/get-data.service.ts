...
getWordOfDay() {
  ...
  getData(word: string): Observable<any[]> {
       ...
       const origin = this.http.get(`https://api.wordnik.com/v4/word.json/${word}/phrases?api_key=${this.key}`);
       const synonyms = this.http.get(`https://api.wordnik.com/v4/word.json/${word}/relatedWords?api_key=${this.key}`);

       return forkJoin([defintionAndPart, origin, synonyms]);
  }
}
